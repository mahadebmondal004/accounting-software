...
<div class="card shadow-sm border-0 fade-in-up">
    <div class="card-header bg-white border-bottom py-3 d-flex justify-content-between align-items-center">
        <h5 class="mb-0 text-primary fw-bold"><i class="fas fa-edit me-2"></i> Edit Sales Invoice</h5>
        <div class="text-muted small fw-bold badge bg-light text-dark border px-3 py-2">
            Date: <?php echo date('d-M-Y'); ?>
        </div>
    </div>
    <div class="card-body p-4">
        <form action="<?php echo APP_URL; ?>/sales/edit/<?php echo $data['voucher']->id; ?>" method="post">
            <?php echo csrf_field(); ?>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="date" name="due_date" class="form-control"
                            value="<?php echo $data['invoice_details']->due_date ?? date('Y-m-d', strtotime('+7 days')); ?>">
                    </div>
                    <div class="col-md-3">
                        ...
                        <select name="customer_ledger_id" class="form-select select2-basic" required autofocus>
                            ...
                            <?php foreach ($data['customers'] as $c): ?>
                                <?php if ($c->type == 'Customer' || $c->nature == 'Assets'): ?>
                                    <option value="<?php echo $c->id; ?>" <?php echo ($data['customer']->id == $c->id) ? 'selected' : ''; ?>>
                    ...
                    <tbody>
                        <?php if (!empty($data['items'])): ?>
                            <?php foreach ($data['items'] as $item): ?>
                                <tr class="inv-row">
                                    <td class="ps-3">
                                        <input type="hidden" name="items[item_id][]" class="item_id" value="<?php echo $item->item_id ?? ''; ?>">
                        ...
                        <?php else: ?>
                            ...
                            <tr class="inv-row">
                                <td class="ps-3">
                                    <input type="hidden" name="items[item_id][]" class="item_id">
                                    <input type="text" name="items[name][]"
                ...
                <div class="col-md-8">
                    <div class="card p-3 bg-light border-0">
                        ...
                        <textarea name="terms_conditions" class="form-control" rows="3"
                            placeholder="Enter terms and conditions..."><?php echo $data['invoice_details']->terms_conditions ?? ''; ?></textarea>
                    </div>
                </div>
                <div class="col-md-4 text-end">
                    <input type="hidden" name="customer_name" id="customerName">
                    <input type="hidden" name="total_amount" id="inpTotal" value="0">
                    <input type="hidden" name="sales_ledger_id" value="<?php echo $data['sales_ledgers'][0]->id ?? ''; ?>">
