...
<div class="card shadow-sm border-0 fade-in-up">
    <div class="card-header bg-white border-bottom py-3 d-flex justify-content-between align-items-center">
        <h5 class="mb-0 text-primary fw-bold"><i class="fas fa-edit me-2"></i> Edit Estimate</h5>
        <div class="text-muted small fw-bold badge bg-light text-dark border px-3 py-2">
            Date: <?php echo date('d-M-Y'); ?>
        </div>
    </div>
    <div class="card-body p-4">
        <form action="<?php echo APP_URL; ?>/estimates/edit/<?php echo $data['estimate']->id; ?>" method="post">
            <?php echo csrf_field(); ?>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-2">
                        ...
                        <input type="date" name="expiry_date" class="form-control"
                            value="<?php echo $data['estimate']->expiry_date ?? date('Y-m-d', strtotime('+30 days')); ?>">
                    </div>
                    <div class="col-md-3">
                        ...
                        <select name="customer_ledger_id" class="form-select select2-basic" required autofocus>
                            ...
                            <?php foreach ($data['customers'] as $c): ?>
                                <?php if ($c->type == 'Customer' || $c->nature == 'Assets'): ?>
                                    <option value="<?php echo $c->id; ?>" <?php echo ($data['estimate']->customer_ledger_id == $c->id) ? 'selected' : ''; ?>>
            ...
            <!-- Ensure total_amount is used for edit, matching controller -->
                ...
                <div class="col-md-8">
                    <div class="card p-3 bg-light border-0">
                        ...
                        <textarea name="notes" class="form-control" rows="3"
                            placeholder="Enter notes and terms..."><?php echo $data['estimate']->notes ?? ''; ?></textarea>
                    </div>
                </div>
                <div class="col-md-4 text-end">
                    <button type="submit" class="btn btn-success px-5 py-3 w-100 shadow-sm fw-bold">
                        <i class="fas fa-save me-2"></i> Update Estimate
                    </button>
                </div>
            </div>
        </form>
...
<script>
    ...
    // Handle Customer Name for narration (removed old listener as element changed)
    // New listener if needed, but narration not explicitly asked for here.
