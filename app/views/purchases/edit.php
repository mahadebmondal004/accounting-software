...
<div class="card shadow-sm border-0 fade-in-up">
    <div class="card-header bg-white border-bottom py-3 d-flex justify-content-between align-items-center">
        <h5 class="mb-0 text-primary fw-bold"><i class="fas fa-edit me-2"></i> Edit Purchase Bill</h5>
        <div class="text-muted small fw-bold badge bg-light text-dark border px-3 py-2">
            Date: <?php echo date('d-M-Y'); ?>
        </div>
    </div>
    <div class="card-body p-4">
        <form action="<?php echo APP_URL; ?>/purchases/edit/<?php echo $data['voucher']->id; ?>" method="post">
            <?php echo csrf_field(); ?>
            <div class="card bg-light border-0 p-3 mb-4">
                <div class="row g-3">
                    <div class="col-md-2">
                        <label class="form-label small fw-bold text-muted">Bill Number</label>
                        ...
                        <select name="supplier_ledger_id" class="form-select select2-basic" required autofocus>
                            ...
                            <?php foreach ($data['suppliers'] as $c): ?>
                                <?php if ($c->type == 'Supplier' || $c->nature == 'Liabilities'): ?>
                                    <option value="<?php echo $c->id; ?>" <?php echo ($data['supplier']->id == $c->id) ? 'selected' : ''; ?>>
                        ...
                        <?php if (!empty($data['items'])): ?>
                            <?php foreach ($data['items'] as $item): ?>
                                <tr class="inv-row">
                                    ...
                                    <!-- Tax Inputs (Values populated by JS or Server if saved, here logic relies on JS calc on load) -->
            ...
            <div class="row align-items-end">
                ...
                <div class="col-md-4 text-end">
                    ...
                    <input type="hidden" name="purchase_ledger_id" value="<?php echo $data['purchase_ledgers'][0]->id ?? ''; ?>">
                    <input type="hidden" name="tax_ledger_id" value="<?php echo $data['tax_ledgers'][0]->id ?? ''; ?>">
                    <button type="submit" class="btn btn-primary px-5 py-3 w-100 shadow-sm fw-bold">
                        <i class="fas fa-save me-2"></i> Update Purchase Bill
                    </button>
                </div>
            </div>
        </form>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        toggleTaxColumns(); // This will trigger calcRow and calcTotals
    });
    ...
    // Handle Supplier Name for narration
